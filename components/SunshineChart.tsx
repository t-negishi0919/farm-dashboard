...
export function SunshineChart({ data }: { data: CombinedRow[] }) {
  const chartData = data.map((r) => ({
    date: r.date.slice(5),
    数量: r.totalQuantity,
    日照時間: r.sunshine != null ? +r.sunshine.toFixed(1) : null,
  }));
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <ComposedChart data={chartData} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
        ...
        <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 10, fill: "rgba(232,240,234,0.35)" }} axisLine={false} tickLine={false} unit="h" />
        ...
        <Line yAxisId="right" type="monotone" dataKey="日照時間" stroke="oklch(0.82 0.14 90)" strokeWidth={2} dot={false} name="日照時間" connectNulls />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
